pipeline {
	...
	stage('Build Docker Image') {
   	    steps {
		    ...
		    sh 'echo "Building Docker Image..."'
     	    	    sh 'docker build -t romango/romanudacitycapstone .'
		}
            }
        }
	...
	stage('Push Image To Dockerhub') {
   	    steps {
			...
			docker push romango/romanudacitycapstone
                    '''
		}
            }
        }
	...
	stage('Create k8s cluster') {
		...
		withAWS(credentials: 'aws-kubectl', region: 'us-west-2a') {
			...
			--name romanudacitycapstoneproject \
			--version 1.14 \
			--region us-west-2 \
	...
	stage('Configure kubectl') {
		...
		withAWS(credentials: 'aws-kubectl', region: 'us-west-2a') {
		    ...
		    sh 'aws eks --region us-west-2 update-kubeconfig --name romanudacitycapstoneproject'
		}
	    }
        }
